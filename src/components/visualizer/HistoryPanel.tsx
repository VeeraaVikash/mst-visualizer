            ...
            <div key={i} onClick={() => onJump(i)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 1 }}>
                  <span style={{ fontSize: 9, fontFamily: "'JetBrains Mono', monospace", color: isCur ? cfg.c : 'var(--text-muted)', textTransform: 'uppercase' }}>{cfg.label}</span>
                  <span style={{ marginLeft: 'auto', fontSize: 9, fontFamily: "'JetBrains Mono', monospace", flexShrink: 0,
                    color: s.type === 'ACCEPT_EDGE' ? 'var(--accent-accept)' : 'var(--text-muted)' }}>
                    {s.type === 'ACCEPT_EDGE' ? `+${s.costDelta}${unit && ' ' + unit}` : `${s.mstCost}${unit && ' ' + unit}`}
                  </span>
                </div>
                <p style={{ fontSize: 10, color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', margin: 0 }}>
                  {s.explanation.substring(0, 52)}{s.explanation.length > 52 ? '…' : ''}
                </p>
              </div>
              <span style={{ fontSize: 8, color: 'var(--text-muted)', flexShrink: 0, fontFamily: "'JetBrains Mono', monospace" }}>
